		...
		<div class="mcart-border">
								...
								<div class="product-details">  
									<?php
										echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
											'<a href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s" data-cart_item_key="%s"> <i class="tb-icon tb-icon-zt-trash"></i> </a>',
									...
									<div class="group">
										...
										<?php 
                                            if (greenmart_tbay_get_config('show_mini_cart_qty', false)) {
												...
												?>
												<span class="quantity">
													<?php esc_html_e('x', 'greenmart');?><?php echo apply_filters('woocommerce_widget_cart_item_quantity',  sprintf('%s', $cart_item['quantity']) , $cart_item, $cart_item_key); ?>
												</span>
												<?php
											}
											?>
			...
			<?php else: ?>
				<ul class="cart_empty <?php echo esc_attr($args['list_class']); ?>">
					<li><span><?php esc_html_e('Your cart is empty','greenmart') ?></span></li>
					<li class="total"><a class="button wc-continue" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>"><?php esc_html_e( 'Continue shopping', 'greenmart' ) ?><i class="tb-icon tb-icon-angle-right"></i></a></li>
				</ul>
			<?php endif; ?>

			<?php if(sizeof(WC()->cart->get_cart()) > 0) : ?>

				<p class="total"><?php esc_html_e('Total', 'greenmart'); ?> <?php echo WC()->cart->get_cart_subtotal(); ?></p>
